...
- **The UDR7's hotspot config only accepts a raw IPv4 address for the external portal server, not a domain** (confirmed: `wifi.basekc.com`, `www.wifi.basekc.com`, and `https://basekc.com` were all rejected). Since a bare IP can't hold a trusted HTTPS certificate, the fix is a tiny plain-HTTP redirector at a static local IP that immediately bounces guests to the real `https://wifi.basekc.com` URL, preserving the query string. It runs alongside the Home Assistant relay on the same local device — see `local-relay/`.
